        ...
        return (
            <div>
                <CSSTransition
                    ...
                    {(open || willClose || willOpen) ? <div className='logCon' style={{ width: vw(90), height: vh(85), marginLeft: vw(5), marginTop: vh(2) }}>
                        {row.map((v, key) => {
                            return <p className='logItem' key={key}>{v.displayName && <span>{v.displayName} &nbsp;:&nbsp;</span>}{v.rawLine}</p>
                        })}
                        <div ref={this.backBtn} >
                            <Abutton onClick={this.toggleLog} type='small' ><span>返回<VerticalAlignBottomOutlined /></span></Abutton>
                        </div>
                    </div> : <div></div>}
                </CSSTransition>
